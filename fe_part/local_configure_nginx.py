...
def get_fe_containers():
    # Run the docker command to get container names
    cmd = "docker ps -a --format '{{.Names}}' | grep '^fe'"
...
def generate_nginx_conf(container_names):
    upstream_block = "upstream frontend {\n"
    for idx, container_name in enumerate(container_names, start=1):
        upstream_block += f"    server {container_name}:80;\n"
    upstream_block += "}\n\n"
...
def write_nginx_conf_to_file(nginx_conf_content, file_path):
    ...
    print()
    print(f"Nginx local  configuration file '{file_path}' updated successfully.")
    print()

if __name__ == "__main__":
    fe_containers = get_fe_containers()
    nginx_conf_content = generate_nginx_conf(fe_containers)
    nginx_conf_path = "/project/nginx_part/config_file"
